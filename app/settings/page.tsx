...
const normalizePercentInput = (value: string) => {
  ...
  return formatPercentNumber(percentStringToNumber(value));
};

const TARGET_TEMPLATE_OPTIONS: Array<{
  id: string;
  label: string;
  ...
  {
    id: "0-250k",
    label: "$0–$250K real revenue",
    allocations: { profit: 5, owners_pay: 50, tax: 15, operating: 30 },
  },
  {
    id: "250k-500k",
    label: "$250K–$500K real revenue",
    allocations: { profit: 10, owners_pay: 35, tax: 15, operating: 30 },
  },
  {
    id: "500k-1m",
    label: "$500K–$1M real revenue",
    allocations: { profit: 10, owners_pay: 20, tax: 15, operating: 55 },
  },
  {
    id: "1m-5m",
    label: "$1M–$5M real revenue",
    allocations: { profit: 10, owners_pay: 15, tax: 15, operating: 60 },
  },
  {
    id: "5m-10m",
    label: "$5M–$10M real revenue",
    allocations: { profit: 15, owners_pay: 10, tax: 15, operating: 60 },
  },
  {
    id: "10m-50m",
    label: "$10M–$50M real revenue",
    allocations: { profit: 20, owners_pay: 0, tax: 15, operating: 65 },
  },
];
...
export default function SettingsPage() {
  ...
  const menuSections = useMemo(
    () => [
      { id: "allocations", label: "Current allocations", description: "Live percentages" },
      { id: "cadence", label: "Allocation cadence", description: "Automation schedule" },
      { id: "rollout", label: "Targets & rollout", description: "Quarterly adjustments" },
            ...
            <div className="grid gap-8 lg:grid-cols-[260px,1fr]">
              ...
              <div className="space-y-8">
                ...
                <section id="allocations" className="space-y-4">
                  <Card
                    title="Current allocation percentages"
                    subtitle="Review and adjust the live distribution across Profit First accounts."
                  >
                    <div className="flex flex-wrap items-center gap-3">
                      <span
                              ...
                              <button
                                ...
                                onClick={async () => {
                                  ...
                                  const confirmed = window.confirm(
                                    `Delete ${account.name}? Allocations and rollout data for this account will be removed.`
                                  );
                                  if (!confirmed) return;
                                  try {
                                    const client = supabase;
                                    const { error: deleteError } = await client
                                      ...
                                      .eq("pf_slug", account.slug);
                                    if (deleteError) throw deleteError;
                                    const { error: currentDeleteError } = await client
                                      .from("allocation_current")
                                      .delete()
                                      .eq("client_id", clientId)
                                      .eq("pf_slug", account.slug);
                                    if (currentDeleteError) throw currentDeleteError;
                                    const { error: rolloutDeleteError } = await client
                                      .from("allocation_rollout_steps")
                                      .delete()
                                      .eq("client_id", clientId)
                                      .eq("pf_slug", account.slug);
                                    if (rolloutDeleteError) throw rolloutDeleteError;
                    ...
                    <p className="mt-4 text-xs text-slate-500">
                      Target percentages now live within the rollout plan below. Use the rollout plan template selector to quickly populate target values when Supabase data is unavailable.
                    </p>

                    <div className="mt-6 flex flex-wrap items-center gap-3">
                      <Button
                        disabled={!clientId || !currentAllocOk}
                        onClick={async () => {
                          if (!clientId || !currentAllocOk) return;
                          if (!supabase) {
                            alert(
                              "Supabase is not configured. Add NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_ANON_KEY to save current allocations."
                            );
                            return;
                          }
                          const client = supabase;
                          try {
                            const payload = accountSlugs.map((slug) => ({
                              client_id: clientId,
                              pf_slug: slug,
                            ...
                            const { error: upsertError } = await client
                              .from("allocation_current")
                              .upsert(payload, { onConflict: "client_id,pf_slug" });
                            if (upsertError) throw upsertError;
                            alert("Current allocations saved.");
                          } catch (err) {
                            console.error("Unable to save current allocations", err);
                            alert("Failed to save current allocations. Check Supabase policies and try again.");
                          }
                        }}
                        className={`border-blue-600 bg-blue-600 text-white hover:bg-blue-500 ${
                          !currentAllocOk ? "cursor-not-allowed opacity-60" : ""
                        }`}
                      >
                      ...
                      <Button
                        onClick={async () => {
                          ...
                          if (!slug) {
                            alert("Account name must include letters or numbers.");
                            return;
                          }
                          const normalized = slug.replace(/[^a-z]/g, "");
                          if (slug.includes("owners_pay") || normalized.includes("ownerspay")) {
                            alert("The core Owner's Pay account already exists and cannot be duplicated.");
                            return;
                          }
                          const existingSlugs = new Set<string>([
                            ...CORE_ACCOUNT_LAYOUT.map((row) => row.slug),
                            ...accounts.map((row) => row.slug),
                          ]);
                          if (existingSlugs.has(slug)) {
                            alert("An account with a similar name already exists.");
                            return;
                          }
                          if (!supabase) {
                            alert(
                ...
                <section id="cadence" className="space-y-4">
                ...
                <section id="rollout" className="space-y-4">
                    <Card title="Target allocations & rollout plan" subtitle="Ease into target allocations over several quarters.">
                      <div className="space-y-6 text-sm text-slate-600">
                        ...
                        <div className="space-y-4">
                          <div className="flex flex-wrap items-center gap-3">
                            ...
                            >
                              Target total: {allocTotalPct.toFixed(1)}%
                            </span>
                            <label className="flex items-center gap-2 text-sm text-slate-600">
                              Apply template
                              <select
                                ...
                                onChange={(e) => {
                                  ...
                                  if (!value) return;
                                  const template = TARGET_TEMPLATE_OPTIONS.find((option) => option.id === value);
                                  if (template) {
                                    setAllocInputs((prev) => {
                                      const next = { ...prev };
                                      accountSlugs.forEach((slug) => {
                                        if (template.allocations[slug] !== undefined) {
                                          next[slug] = formatPercentNumber(template.allocations[slug]!);
                                ...
                                className="rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm"
                              >
                                <option value="">Select recommended mix</option>
                              ...
                              <div key={account.slug} className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
                                <div className="flex items-center justify-between text-sm font-medium text-slate-700">
                                  ...
                                  <span className="text-xs text-slate-500">
                                    Current {currentAllocInputs[account.slug] ?? allocInputs[account.slug] ?? "0"}%
                                  </span>
                                </div>
                                <div className="mt-2 flex items-center gap-2">
                                  <input
                          ...
                          <div className="flex flex-wrap items-center gap-3">
                            <Button
                              ...
                              onClick={async () => {
                                ...
                                if (!supabase) {
                                  alert(
                                    "Supabase is not configured. Add NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_ANON_KEY to save target allocations."
                                  );
                                  return;
                                }
                                const client = supabase;
                                try {
                                  ...
                                  alert("Target allocations saved.");
                                } catch (err) {
                                  console.error("Unable to save target allocations", err);
                                  alert("Failed to save target allocations. Check Supabase policies and try again.");
                                }
                              }}
                              className={`border-blue-600 bg-blue-600 text-white hover:bg-blue-500 ${
                            ...
                            <Button onClick={() => clientId && loadClientData(clientId)}>Refresh target data</Button>
                          </div>
                        </div>
                                      ...
                                      {rolloutRows.map((row, rowIdx) => (
                                        <td key={`${account.slug}-${row.quarter}`} className="px-3 py-2">
                                          <input
                                            ...
                                            onChange={(e) => {
                                              const sanitized = sanitizePercentInput(e.target.value);
                                              handleRolloutCellChange(rowIdx, account.slug, sanitized);
                                            }}
                                            onBlur={(e) => {
                                              const normalized = normalizePercentInput(e.target.value);
                                              handleRolloutCellChange(rowIdx, account.slug, normalized);
                                            }}
                                            className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm"
                                          />
                                        </td>
                        ...
                        <div className="flex flex-wrap items-center gap-3">
                          <Button
                            disabled={!clientId || !allocOk || !currentAllocOk || rolloutHasError}
                            onClick={async () => {
                              ...
                              if (!allocOk) {
                                alert("Target allocations must total 100% before saving the rollout plan.");
                                return;
                              }
                              if (!currentAllocOk) {
                              ...
                              try {
                                ...
                                if (rolloutRows.length) {
                                  const inserts = rolloutRows.flatMap((row, idx) =>
                                ...
                                alert("Target allocations and rollout plan saved.");
                              } catch (error: any) {
                                console.error("Unable to save rollout plan", error);
                                if (error?.code === "42P01") {
                                  alert(
                                    "Run the latest Supabase migration to add allocation_targets and allocation_rollout_steps tables."
                                  );
                                } else {
                            ...
                            className={`border-blue-600 bg-blue-600 text-white hover:bg-blue-500 ${
                              !clientId || !allocOk || !currentAllocOk || rolloutHasError ? "cursor-not-allowed opacity-60" : ""
                            }`}
                          >
                            Save targets & rollout
                          </Button>
                ...
                <section id="profit" className="space-y-4">
                        ...
                        <div className="grid gap-3 md:grid-cols-2">
                          <label className="flex flex-col gap-1">
                            ...
                            <input
                              ...
                              onChange={(e) => {
                                const sanitized = sanitizePercentInput(e.target.value);
                                setProfitSettings((prev) => ({ ...prev, bonusPct: sanitized }));
                              }}
                              onBlur={(e) => {
                                const normalized = normalizePercentInput(e.target.value);
                                setProfitSettings((prev) => ({ ...prev, bonusPct: normalized }));
                              }}
                              placeholder="0"
                              className="rounded-lg border border-slate-300 px-3 py-2"
                            />
                          ...
                          <label className="flex flex-col gap-1">
                            ...
                            <input
                              ...
                              onChange={(e) => {
                                const sanitized = sanitizePercentInput(e.target.value);
                                setProfitSettings((prev) => ({ ...prev, vaultPct: sanitized }));
                              }}
                              onBlur={(e) => {
                                const normalized = normalizePercentInput(e.target.value);
                                setProfitSettings((prev) => ({ ...prev, vaultPct: normalized }));
                              }}
                              placeholder="0"
                              className="rounded-lg border border-slate-300 px-3 py-2"
                            />
                      ...
                      <div className="space-y-4 text-sm text-slate-600">
                          ...
                          <div className="grid gap-3 md:grid-cols-2">
                            <label className="flex flex-col gap-1">
                              ...
                              <input
                                ...
                                onChange={(e) => {
                                  const sanitized = sanitizePercentInput(e.target.value);
                                  setTaxSettings((prev) => ({ ...prev, taxRate: sanitized }));
                                }}
                                onBlur={(e) => {
                                  const normalized = normalizePercentInput(e.target.value);
                                  setTaxSettings((prev) => ({ ...prev, taxRate: normalized }));
                                }}
                                placeholder="0"
                                className="rounded-lg border border-slate-300 px-3 py-2"
                              />
                        ...
                        <label className="flex flex-col gap-1">
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              const sanitized = sanitizePercentInput(e.target.value);
                              setTaxSettings((prev) => ({ ...prev, vaultPct: sanitized }));
                            }}
                            onBlur={(e) => {
                              const normalized = normalizePercentInput(e.target.value);
                              setTaxSettings((prev) => ({ ...prev, vaultPct: normalized }));
                            }}
                            placeholder="0"
                            className="rounded-lg border border-slate-300 px-3 py-2"
                          />
